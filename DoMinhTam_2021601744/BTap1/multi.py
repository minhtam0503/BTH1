...
class Multi:
    ...
    def output_matrix(self):
        ...
        # button
        self.frame_button = Frame(self.gui_multi_output, bg='#F9E79F', highlightbackground='black',
        ...
        for i in range(self.rows_a):
            for j in range(self.cols_a):
                Label(canvas_frame, text=self.matrix_a[i][j], font=('arial', 12, 'bold'), bd=5).grid(row=i,
                                                                                                     column=j + 1,
                                                                                                     sticky='news',
                                                                                                     padx=5, pady=5)

        # display matrix_b input
        Label(canvas_frame, text='Matrix B:', font=('arial', 14, 'bold'), underline=0).grid(row=0,
                                                                                            column=self.cols_a + 1)

        for i in range(self.rows_b):
            for j in range(self.cols_b):
                Label(canvas_frame, text=self.matrix_b[i][j], font=('arial', 12, 'bold'), bd=5).grid(row=i,
                                                                                                     column=j + self.cols_a + 2)

        # display product
        Label(canvas_frame, text='Product:', font=('arial', 14, 'bold'), underline=0).grid(row=self.rows_a * 3,
                                                                                           column=0)
        ...
        for i in range(self.rows_a):
            for j in range(self.cols_b):
                Label(canvas_frame, text=self.product_matrix[i][j], font=('arial', 12, 'bold'), bd=5).grid(
                    row=i + self.rows_a * 2, column=j + 1)
    ...
    def input_matrix(self):
        ...
        # create input frame
        self.frame_multi_input = Frame(self.gui_multi_input, bg='#F9E79F', highlightbackground='black',
                                       highlightthickness=1, padx=5, pady=5)
        ...
        for i in range(self.rows_a):
            ...
            for j in range(self.cols_a):
                ...
                entries[i].append(
                    Entry(self.frame_multi_input, textvariable=text_var[i][j], width=10, font=('arial', 10, 'bold')))
        ...
        Label(self.frame_multi_input, text="Matrix B:", font=('arial', 12, 'bold'), bg='#F9E79F').grid(
            row=self.rows_a + 1, column=0)
        ...
        for i in range(self.rows_b):
            ...
            for j in range(self.cols_b):
                ...
                entries_b[i].append(
                    Entry(self.frame_multi_input, textvariable=text_var_b[i][j], width=10, font=('arial', 10, 'bold')))
                entries_b[i][j].grid(row=i + self.rows_a + 2, column=j + 1)
                Label(self.frame_multi_input, text=i + 1, bg='#F9E79F').grid(row=i + self.rows_a + 2, column=0,
                                                                             sticky='e')
        ...
        self.frame_button = Frame(self.gui_multi_input, bg='#F9E79F', highlightbackground='black', highlightthickness=1,
                                  padx=5, pady=5)
        self.frame_button.pack(fill='x', expand=True, padx=5, pady=3)
        Button(self.frame_button, text="Enter", font=('arial', 10, 'bold'), width=11, activebackground='green', padx=10,
               pady=5, command=get_mat).grid(row=0, column=2, padx=14)
        Button(self.frame_button, text="Back", font=('arial', 10, 'bold'), width=11, activebackground='green', padx=14,
               command=self.back_to_dimensions).grid(row=0, column=1, padx=14)
        Button(self.frame_button, text="Back to Menu", font=('arial', 10, 'bold'), activebackground='green', pady=10,
               padx=5, command=self.back_to_menu_from_input).grid(row=0, column=0, padx=14)
    ...
    def __init__(self):
        ...
        # prompt dimensions
        Label(self.frame_multi_menu, text='Matrix A dimensions:', font=('arial', 14, 'bold'), bg='#F9E79F').grid(row=0,
                                                                                                                 column=0,
                                                                                                                 columnspan=3,
                                                                                                                 sticky='news',
                                                                                                                 pady=10)
        Label(self.frame_multi_menu, text='Matrix B dimensions:', font=('arial', 14, 'bold'), bg='#F9E79F').grid(row=2,
                                                                                                                 column=0,
                                                                                                                 columnspan=3,
                                                                                                                 sticky='news',
                                                                                                                 pady=10)
        ...
        label = Label(self.frame_multi_menu, textvariable=selected_value, font=('arial', 10, 'bold'), padx=5, pady=5,
                      bg='#F9E79F').grid(row=3, column=0)
        ...
        Button(self.frame_multi_menu, text='Enter', font=('arial', 10, 'bold'), activebackground='green', padx=10,
               pady=5, command=self.input_matrix).grid(row=4, column=0, columnspan=3, pady=10)
        Button(self.frame_multi_menu, text="Back to Menu", font=('arial', 10, 'bold'), activebackground='green',
               pady=10, padx=5, command=self.back_to_menu_from_dimensions).grid(row=5, column=0, columnspan=3)
